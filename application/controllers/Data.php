...
class Data extends CI_Controller {
	...
	public function rusak()
	{
        ...
        $this->data['idbo'] = $this->session->userdata('ses_id');
		$this->data['rusak'] =  $this->db->query("SELECT * FROM tbl_rusak ORDER BY id_rusak DESC");

		if(!empty($this->input->get('id'))){
			$id = $this->input->get('id');
			$count = $this->M_Admin->CountTableId('tbl_rusak','id_rusak',$id);
			if($count > 0)
			{	
				$this->data['rusak'] = $this->db->query("SELECT *FROM tbl_rusak WHERE id_rusak='$id'")->row();
			}else{
	...
	{
		$this->data['idbo'] = $this->session->userdata('ses_id');
		$count = $this->M_Admin->CountTableId('tbl_rusak','id_rusak',$this->uri->segment('3'));
		if($count > 0)
		{
			$this->data['rusak'] = $this->M_Admin->get_tableid_edit('tbl_rusak','id_rusak',$this->uri->segment('3'));
			$this->data['kats'] =  $this->db->query("SELECT * FROM tbl_kategori ORDER BY id_kategori DESC")->result_array();
			$this->data['rakbuku'] =  $this->db->query("SELECT * FROM tbl_rak ORDER BY id_rak DESC")->result_array();
	...
	public function prosesbukurusak()
	{
		if(!empty($this->input->get('buku_id')))
		{
			...
			$buku = $this->M_Admin->get_tableid_edit('tbl_rusak','id_rusak',htmlentities($this->input->get('buku_id')));
			...
			$this->M_Admin->delete_table('tbl_rusak','id_rusak',$this->input->get('buku_id'));
		...
		if(!empty($this->input->post('tambah')))
		{
			...
			// load library upload
			$this->load->library('upload',$config);
			$buku_id = $this->M_Admin->buat_kode('tbl_rusak','BK','id_rusak','ORDER BY id_rusak DESC LIMIT 1'); 
		...
		if(!empty($this->input->post('edit')))
		{
			...
			$this->db->where('id_rusak',htmlentities($post['edit']));
...
public function pdt(){
	$this->load->library('dompft_gen');
	$data['buku'] = $this->M_Admin->get_tableid_edit('tbl_buku') -> result;

	$this->load->view('buku/laporan_pdf', $data);

	$papersize = 'A4';
	$orientation= 'landscape';
	$html = $this->output->get_output();
	$this->dompdf->set_paper($set_paper, $orientation);
	$this->dompdt->loadhtlm($html);
	$this->dompdf->render();
